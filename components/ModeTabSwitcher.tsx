...
export default function ModeTabSwitcher({ currentMode, totalCount }: ModeTabSwitcherProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-2">
      <div className="bg-slate-100/80 dark:bg-slate-900/80 backdrop-blur-sm p-1.5 sm:p-2 rounded-2xl sm:rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors">
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-bold transition-all duration-200 cursor-pointer active:scale-95 touch-manipulation ${
              currentMode === 'all'
                ...
                : 'bg-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/60 dark:hover:bg-slate-800/60 border border-transparent'
            }`}
          >
        ...
        <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl sm:rounded-2xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/60 text-slate-600 dark:text-slate-300 text-xs leading-relaxed shadow-2xs">
          {currentMode === 'top10' ? (
            <>
              <Flame className="w-4 h-4 text-amber-500 shrink-0" />
              <span>{t.mode.top10Desc}</span>
            </>
          ) : (
            <>
              <ShieldCheck className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0" />
              <span>{t.mode.allDesc}</span>
            </>
          )}
        </div>
